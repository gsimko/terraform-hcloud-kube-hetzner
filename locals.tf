locals {
  ...
  kustomization_backup_yaml = yamlencode({
    ...
    resources = concat(
      [
        "https://github.com/hetznercloud/hcloud-cloud-controller-manager/releases/download/${local.ccm_version}/ccm.yaml",
...
EOT
  ...
  agent_cidr_ranges  = [for index in range(200) : cidrsubnet(var.network_ipv4_cidr, 16, index)]
  control_cidr_ranges  = [for index in range(56) : cidrsubnet(var.network_ipv4_cidr, 16, 255 - index)]
