...
resource "null_resource" "kustomization" {
  ...
  provisioner "remote-exec" {
    inline = [
      "set -ex",
      "kubectl -n kube-system create secret generic hcloud --from-literal=token=${var.hcloud_token} --from-literal=network=${data.hcloud_network.k3s[0].name} --dry-run=client -o yaml | kubectl apply -f -",
