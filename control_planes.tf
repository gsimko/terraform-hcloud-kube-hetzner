module "control_planes" {
  ...
  ipv4_subnet_id               = var.use_private_network ? hcloud_network_subnet.control_plane[each.value.nodepool_index].id : null
  ...
  private_ipv4 = var.use_private_network ? cidrhost(hcloud_network_subnet.control_plane[each.value.nodepool_index].ip_range, each.value.index + 101) : cidrhost(local.control_cidr, each.value.index)
  ...
  k3s-config = { for k, v in local.control_plane_nodes : k => merge(
    {
      ...
      node-ip                     = var.use_private_network ? module.control_planes[k].private_ipv4_address : module.control_planes[k].ipv4_address
      advertise-address           = var.use_private_network ? module.control_planes[k].private_ipv4_address : module.control_planes[k].ipv4_address
      node-label                  = v.labels
      node-taint                  = v.taints
      selinux                     = true
